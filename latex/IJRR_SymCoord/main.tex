...
\midrule
\textbf{PRISM (symbiotic)} & $\mathbf{5.40 \pm 1.98}$ & [5.05, 5.77] & $\mathbf{+40.9\%}$ \\
Task-only                   & $5.06 \pm 1.90$          & [4.72, 5.40] & $+32.0\%$ \\
Flat-cooperative            & $4.88 \pm 2.00$          & [4.53, 5.25] & $+27.4\%$ \\
Heuristic oracle            & $3.83 \pm 1.27$          & [3.40, 4.30] & --- \\
\bottomrule
\multicolumn{4}{l}{\small PRISM vs.\ flat-coop: Mann-Whitney $U$, $p{=}0.013$; Welch $t$, $p{=}0.27$ (n.s.)} \\
\multicolumn{4}{l}{\small Task-only vs.\ flat-coop: $p{=}0.57$ (n.s.) — cooperative bonus provides no benefit} \\
\end{tabular}
\end{table}

PRISM achieves the highest mean throughput (5.40~del/ep), outperforming both flat-cooperative ($+10.6\%$, Mann-Whitney $p{=}0.013$) and task-only ($+6.7\%$). A key finding is that task-only (5.06~del/ep) outperforms flat-cooperative (4.88~del/ep, $p{=}0.57$, n.s.), indicating that the undifferentiated team bonus in flat-cooperative provides no benefit over pure task reward. Only PRISM's structured relationship typing produces a statistically detectable throughput gain. All trained conditions substantially exceed the heuristic oracle ($+27.4\%$ to $+40.9\%$).
