...
Experiments in a battery-constrained heterogeneous warehouse (TARWARE, 4 AGVs + 2 pickers) show that PRISM achieves 5.40 deliveries per episode versus 4.88 for flat-cooperative (+10.6\%, Mann-Whitney $p{=}0.013$) and 5.06 for task-only (+6.7\%), with all trained conditions substantially outperforming a hand-crafted heuristic oracle ($+40.9\%$). Relationship emergence analysis confirms mutualism and commensalism as the dominant observed ecological types; the commensalism fraction grows monotonically over training, indicating agents learn battery-coordinated behaviour. Notably, flat-cooperative reward (undifferentiated team bonus) provides no benefit over task-only reward ($\Delta{=}$-0.18~del/ep), confirming that the structured relationship typing in PRISM---not the mere presence of a cooperative bonus---is responsible for the observed gain.
\end{abstract}
...
A hand-crafted BFS scheduler with greedy task assignment serves as a practical upper bound, evaluated for 30 episodes (seed 42) on the full package mix (20\% SOLO, 30\% STANDARD, 25\% HEAVY, 15\% PICKER\_SOLO, 10\% LARGE). LARGE packages require simultaneous 2-AGV + 2-picker coordination and are not delivered by any learned policy in this study; they are included in the request queue to test robustness but excluded from the delivery count discussion. This oracle uses no learning and represents the best achievable under rule-based routing with full state knowledge.
...
\textbf{Coverage bonus.} All three conditions include a small shared bonus $w_{\mathrm{cov}} \cdot N_{\mathrm{del}}$ per delivery of any package type, added to all agents. This prevents degenerate specialisation on a single package type (STANDARD) at the expense of ignoring SOLO, HEAVY, and PICKER\_SOLO requests, which would otherwise dominate due to asymmetric per-type reward allocation. With the coverage bonus, agents deliver a realistic mix of the four deliverable package types.
...
\begin{table}[htbp]
\centering
\caption{Deliveries per episode (20 episodes $\times$ 6 seeds = 120 total per condition). 95\% bootstrap CI. All trained conditions surpass the heuristic oracle.}
\label{tab:throughput}
\begin{tabular}{lccc}
\toprule
\textbf{Condition} & \textbf{Mean $\pm$ SD} & \textbf{95\% CI} & \textbf{vs.\ Heuristic} \\
\midrule
\textbf{PRISM (symbiotic)} & $\mathbf{11.37 \pm 2.17}$ & [10.97, 11.76] & $\mathbf{+196.5\%}$ \\
Task-only                   & $10.57 \pm 2.12$           & [10.20, 10.96] & $+175.9\%$ \\
Flat-cooperative            & $10.24 \pm 2.09$           & [9.87, 10.61]  & $+167.4\%$ \\
Heuristic oracle            & $3.83 \pm 1.27$            & [3.40, 4.30]   & --- \\
\bottomrule
\multicolumn{4}{l}{\small PRISM vs.\ flat-coop: Mann-Whitney $U{=}9147$, $p{=}0.0001$; Welch $t{=}2.99$, $p{=}0.015$; Cohen's $d{=}0.53$} \\
\multicolumn{4}{l}{\small Task-only vs.\ flat-coop: $p{=}0.47$ (n.s.) — cooperative bonus provides no benefit} \\
\end{tabular}
\end{table}

PRISM achieves the highest mean throughput (11.37~del/ep), outperforming flat-cooperative by $+11.0\%$ (Mann-Whitney $p{=}0.0001$, Welch $p{=}0.015$, Cohen's $d{=}0.53$, medium effect) and task-only by $+7.6\%$. A key finding is that task-only (10.57~del/ep) and flat-cooperative (10.24~del/ep) do not differ significantly ($p{=}0.47$, n.s.), indicating that an undifferentiated team bonus provides no benefit over pure task reward. Only PRISM's structured relationship typing produces a statistically detectable throughput gain. All trained conditions substantially exceed the heuristic oracle.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figure/fig2_throughput.pdf}
  \caption{Throughput comparison (deliveries per episode, mean $\pm$ SD, 120 episodes per condition). PRISM achieves the highest mean throughput (11.37~del/ep), outperforming flat-cooperative by 11.0\% (Mann-Whitney $p{=}0.0001$) and task-only by 7.6\%. All trained conditions substantially exceed the heuristic oracle (3.83~del/ep).}
  \label{fig:throughput}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figure/fig3_learning_curves.pdf}
  \caption{Learning curves (deliveries per episode, smoothed, $\pm$\,1\,SD across seeds) for all three conditions and heuristic reference line (dotted). PRISM converges to the highest delivery rate.}
  \label{fig:learning_curves}
\end{figure}

\subsection{Package Type Delivery Breakdown}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figure/fig4_package_breakdown.pdf}
  \caption{Mean deliveries per episode by package type and condition (20 episodes $\times$ 6 seeds). PRISM's advantage is concentrated in STANDARD packages (AGV + picker joint delivery, i.e.\ mutualistic interactions), with a $+0.69$\,del/ep gain over flat-cooperative ($+41\%$). Single-role tasks (HEAVY and PICKER\_SOLO) are delivered at equivalent rates across all conditions, confirming that symbiotic shaping promotes inter-role coordination without disrupting single-agent tasks.}
  \label{fig:pkg_breakdown}
\end{figure}

PRISM's throughput advantage is not uniform across package types. As shown in Fig.~\ref{fig:pkg_breakdown}, the gains are concentrated in STANDARD packages, which require simultaneous AGV delivery and picker unloading — the canonical mutualistic interaction in this environment. PRISM delivers $2.38$ STANDARD packages per episode compared to $1.69$ for flat-cooperative ($+41\%$) and $1.92$ for task-only. In contrast, HEAVY packages (AGV-only) and PICKER\_SOLO packages (picker-only) are delivered at statistically equivalent rates across all conditions (${\leq}0.11$ del/ep difference). This pattern is the empirical signature of PRISM's design: the symbiotic reward directly incentivises the AGV-picker coordination required for STANDARD tasks, and this benefit does not come at the cost of single-role task performance.

\subsection{Experiment 2: Resilience Under Agent Failure (C3)}

\begin{table}[htbp]
\centering
\caption{Team resilience: full-team vs.\ one-AGV-failed performance (5 episodes $\times$ 6 seeds per condition). All conditions show similar resilience under v4 hyperparameters.}
\label{tab:resilience}
\begin{tabular}{lccc}
\toprule
\textbf{Condition} & \textbf{Full team} & \textbf{1 AGV failed} & \textbf{Drop (\%)} \\
\midrule
PRISM (symbiotic)  & 4.57 & 3.70 & 19.0\% \\
Flat-cooperative   & 4.47 & 3.60 & 19.4\% \\
Task-only          & 4.10 & 3.47 & 15.4\% \\
\bottomrule
\multicolumn{4}{l}{\small No significant difference between conditions ($p{>}0.5$, Welch's $t$-test)} \\
\end{tabular}
\end{table}

Under the v4 training configuration (entropy coefficient 0.05), all three conditions show similar resilience to agent failure (${\sim}19\%$ performance drop). The higher exploration induced by the increased entropy coefficient appears to make all policies more adaptable to team changes, equalising fault tolerance across reward structures. The throughput advantage (Table~\ref{tab:throughput}) is therefore the primary differentiator between conditions.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figure/fig5_relationship_emergence.pdf}
...
\subsection{RQ3: Throughput Advantage as the Primary C3 Finding}
\label{sec:disc_rq3}

Table~\ref{tab:throughput} shows that PRISM achieves 5.40~del/ep versus 4.88 for flat-cooperative (+10.6\%, Mann-Whitney $p{=}0.013$) and 5.06 for task-only (+6.7\%). All trained conditions substantially exceed the heuristic oracle (+27.4\%–40.9\%), demonstrating that learned coordination surpasses rule-based routing in a realistic mixed-package environment.

A particularly informative finding is that task-only (5.06~del/ep) outperforms flat-cooperative (4.88~del/ep, $p{=}0.57$, n.s.). This means adding an undifferentiated cooperative bonus does not help agents coordinate better than pure task reward---it may even introduce conflicting gradients. Only PRISM's structured relationship typing produces a statistically detectable gain ($p{=}0.013$). This strongly supports the C2 claim: the \emph{type} of shaping matters, not merely the \emph{presence} of a bonus.

The advantage of PRISM over flat-cooperative is detectable at the episode level (Mann-Whitney) but not at the seed level (Welch $t$-test, $p{=}0.27$, 6 seeds providing limited power). Stronger statistical evidence would require more seeds or a longer training horizon.

Under the current v4 hyperparameters, resilience to agent failure is similar across all three conditions (${\sim}19\%$ drop, Table~\ref{tab:resilience}). The higher exploration induced by entropy coefficient 0.05 makes all policies inherently more adaptable, equalising fault tolerance. This finding suggests that resilience advantage may be more sensitive to training configuration than reward structure, and motivates future investigation of the interaction between entropy and symbiotic shaping.
...
\textbf{C3 (throughput advantage).} On the same team (4 AGVs + 2 pickers), same environment, and same training budget (1\,M steps, 6 seeds), PRISM achieves 5.40~del/ep versus 4.88 for flat-cooperative (+10.6\%, Mann-Whitney $p{=}0.013$) and 5.06 for task-only (+6.7\%). All trained conditions substantially outperform the heuristic oracle (up to +40.9\% for PRISM). A key finding is that flat-cooperative (undifferentiated team bonus) performs worse than task-only ($p{=}0.57$, n.s.), confirming that the cooperative bonus alone does not help: only PRISM's structured relationship typing produces a measurable gain.

The core finding is that the benefit of symbiotic reward shaping comes specifically from \emph{typed} shaping, not from any cooperative bonus. This distinction is the empirical contribution of C3: if flat-cooperative had matched PRISM, the ecological framing would be unnecessary. It does not---and the gap is statistically detectable. Future work with more seeds (${\geq}$10) would strengthen the seed-level statistical evidence.
